  ...
  return (
    <section className="course-sizing" aria-labelledby="course-sizing-title">
      <div className="course-sizing-heading">
        <div><Ruler size={18} /><div><span className="step-label">{c.step}</span><h2 id="course-sizing-title">{c.title}</h2></div></div>
      ...
      <div className="course-sizing-layout">
        <div className="course-sizing-data">
          ...
          <p>{c.target} <strong>{firstLegMinutes} {c.minutes}</strong> {c.targetEnd} {c.notes[courseType]}</p>
          <small className="course-sizing-disclaimer">{c.disclaimer}</small>
        </div>

        {hasPoint ? courseType === 'IODA' ? <IodaCoursePreview key={`IODA:${latitude}:${longitude}`}
          latitude={latitude}
          longitude={longitude}
          axis={courseAxis}
