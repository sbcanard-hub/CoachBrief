  ...
  return (
    <section className="course-sizing" aria-labelledby="course-sizing-title">
      <div className="course-sizing-heading">
        <div><Ruler size={18} /><div><span className="step-label">Dimensionnement</span><h2 id="course-sizing-title">Premier bord recommandé</h2></div></div>
        <span>Estimation coach · à calibrer</span>
      </div>

      <div className="course-sizing-layout">
        <div className="course-sizing-data">
          <div className="course-sizing-main"><strong>{sizing.firstLegNm.toFixed(2).replace('.', ',')} <small>NM</small></strong><span>≈ {sizing.firstLegMeters} m</span></div>
          ...
          <small className="course-sizing-disclaimer">Ce calcul est une aide de préparation, pas une longueur réglementaire. La flotte, le clapot, le courant, la visibilité et la zone disponible restent prioritaires.</small>
        </div>

        {hasPoint ? <CoursePreview latitude={latitude} longitude={longitude} axis={courseAxis} windwardOffset={windwardOffset} firstLegNm={sizing.firstLegNm} courseType={courseType} /> : <div className="course-preview-empty">Placez précisément le plan d’eau pour afficher le premier bord sur la carte.</div>}
      </div>
    </section>
  )
}
