...
const additionalTranslations: Record<string, [string, string, string]> = {
  ...
  'Ex. Antibes → Calvi': ['E.g. Antibes → Calvi', 'Es. Antibes → Calvi', 'Ej. Antibes → Calvi'],
  'Ex. Fastnet, Spi Ouest offshore…': ['E.g. Fastnet, Spi Ouest offshore…', 'Es. Fastnet, Spi Ouest offshore…', 'Ej. Fastnet, Spi Ouest offshore…'],
  ...
  'Le contrôle côtier est indisponible. Réessaie plus tard : aucune route ne peut être proposée sans vérifier les traversées de terre.': ['Coastline checks are unavailable. Try again later: no route can be offered without checking for land crossings.', 'Il controllo costiero non è disponibile. Riprova più tardi: non si può proporre una rotta senza verificare gli attraversamenti della terra.', 'El control de la costa no está disponible. Inténtalo más tarde: no se puede proponer una ruta sin comprobar los cruces de tierra.'],
  'La ligne en pointillés représente uniquement la distance géométrique directe entre D et A ; elle n’est pas une route navigable.': ['The dashed line shows only the geometric distance from start to finish; it is not a navigable route.', 'La linea tratteggiata mostra solo la distanza geometrica tra partenza e arrivo; non è una rotta navigabile.', 'La línea discontinua muestra solo la distancia geométrica entre salida y llegada; no es una ruta navegable.'],
  'Aucune trajectoire maritime n’a pu quitter D : les premiers pas traversent une côte. Vérifie la position du départ et réessaie avec un pas de temps plus court.': ['No marine route could leave the start: initial legs cross land. Check the start position and try a shorter time step.', 'Nessuna rotta marittima può lasciare la partenza: i primi tratti attraversano la costa. Controlla la posizione e riduci il passo temporale.', 'Ninguna ruta marítima puede salir: los primeros tramos cruzan tierra. Comprueba la posición de salida y usa un paso temporal menor.'],
  'Aucune trajectoire isochrone n’a pu être générée dès le départ. Vérifie la date/heure, la météo et la polaire au point D.': ['No isochrone route could be generated from the start. Check the date and time, weather and polar at the start.', 'Impossibile generare una rotta dalla partenza. Controlla data, ora, meteo e polare.', 'No se pudo generar una ruta desde la salida. Comprueba fecha, hora, meteorología y polar.'],
  ...
  'Carte interactive de la route au large': ['Interactive offshore route map', 'Mappa interattiva della rotta d’altura', 'Mapa interactivo de la ruta de altura'],
  'Petit trajet côtier': ['Short coastal trip', 'Breve tratta costiera', 'Trayecto costero corto'],
  'Trajet côtier court': ['Coastal trip', 'Tratta costiera', 'Trayecto costero'],
  'Trajet intermédiaire': ['Intermediate trip', 'Tratta intermedia', 'Trayecto intermedio'],
  'Route au large': ['Offshore route', 'Rotta d’altura', 'Ruta de altura'],
  'Réglage standard': ['Standard settings', 'Impostazioni standard', 'Ajustes estándar'],
  'Temps théorique à': ['Theoretical time at', 'Tempo teorico a', 'Tiempo teórico a'],
  '· hors contraintes de navigation': ['· excluding navigation constraints', '· senza vincoli di navigazione', '· sin restricciones de navegación'],
  'nm : réglage automatique': ['nm: automatic settings', 'nm: impostazioni automatiche', 'nm: ajustes automáticos'],
  'h. Tu peux le modifier manuellement.': ['h. You can adjust it manually.', 'h. Puoi modificarlo manualmente.', 'h. Puedes modificarlo manualmente.'],
  'tronçon': ['leg', 'tratta', 'tramo'],
  'tronçons': ['legs', 'tratte', 'tramos'],
  '5 jours': ['5 days', '5 giorni', '5 días'],
