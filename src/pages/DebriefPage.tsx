...
export function DebriefPage() {
  ...
  return <main className="debrief-page">
    <header className="debrief-hero"><div><button className="debrief-back" onClick={() => navigate('/briefings')}><ArrowLeft size={15}/>{t('backToBriefings')}</button><span>{t('attachedToBriefing')}</span><h1>{t('raceDebrief')}</h1><p>{item.name} · {item.request.location}</p></div><div className="debrief-actions"><button onClick={() => window.print()}><Printer size={15}/>{t('print')}</button><button onClick={remove}><Trash2 size={15}/>{t('deleteDebrief')}</button></div></header>
    {status && <p className="debrief-status" role="status">{status}</p>}
    <section className="debrief-section"><h2>{t('whatHappened')}</h2><div className="debrief-fields">{fields.map(([field, label, kind]) => <label key={field}><span>{label}</span>{kind === 'textarea' ? <textarea rows={3} value={draft[field]} onChange={(e) => update(field, e.target.value)}/> : kind === 'select' ? <select value={draft[field]} onChange={(e) => update(field, e.target.value)}><option value="">—</option><option value="Comité">{t('committee')}</option><option value="Neutre">{t('neutral')}</option><option value="Pin">Pin</option></select> : <input type={field === 'actualStartTime' ? 'time' : 'text'} inputMode={['windSpeed','windDirection','gust'].includes(field) ? 'decimal' : undefined} value={draft[field]} onChange={(e) => update(field, e.target.value)}/>}</label>)}</div></section>
